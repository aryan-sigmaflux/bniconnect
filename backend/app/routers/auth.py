...
from app.utils.helpers import validate_phone, normalize_phone
...
async def send_otp(
    ...
    if existing_user is None:
        if settings.DEV_MODE:
            logger.warning(f"DEV MODE: Allowing OTP for unregistered number {phone}")
        else:
            raise HTTPException(
    ...
    if auth_service._is_bypass_phone(phone):
        logger.warning(f"DEV MODE: Skipping OTP send for {phone[-4:]} — use any 6 digits")
        return SendOTPResponse(message="OTP sent")

    # Generate and store OTP
    otp_code = await auth_service.create_otp(db, phone)

    # Send via WhatsApp
    sent = await whatsapp_service.send_otp(phone, otp_code)
    if not sent:
        logger.error(f"WhatsApp OTP delivery failed for {phone[-4:]}")
        raise HTTPException(
...
async def verify_otp(
    ...
    # Verify OTP
    is_valid = await auth_service.verify_otp(db, phone, body.otp)
    if not is_valid:
        raise HTTPException(
